...
async function InstaDl(url) {
  try {
    const api = await Request('https://nayan-video-downloader.vercel.app/ndown', 
    { params: { url: url }});
...
async function TikTok(url) {
  try {
    const api = await Request('https://www.tikwm.com/api', {
      params: { url: url }
    });
...
async function twitterDl(url) {
  try {
    const api = await Request('https://api.deline.web.id/downloader/twitter', 
    { params: { url: url }});
...
async function Gdrive(url) {
  try {
    const api = await Request('https://api.deline.web.id/downloader/gdrive', 
    { params: { url: url }});
...
async function CapCut(url) {
  try {
    const api = await Request('https://api.vreden.my.id/api/v1/download/capcut', 
    { params: { url: url }});
...
async function Fdroid(url) {
  try {
    const api = await Request('https://api.vreden.my.id/api/v1/download/fdroid', 
    { params: { url: url }});
...
async function Threads(url) {
  try {
    const api = await Request('https://api.vreden.my.id/api/v1/download/threads',
    { params: { url: url }});
...
module.exports = { YtVideo, YtAudio, InstaDl, pinterestDl, TikTok, FaceBook, Kwai, twitterDl, Gdrive, CapCut, Fdroid, Threads };